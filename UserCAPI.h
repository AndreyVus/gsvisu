...
#define GS_POWER_CMD_OSY_FLASHLOADER        5

#define GS_STANDBY_WAKEUP_IGNITION          (1<<0)
#define GS_STANDBY_WAKEUP_KEYS              (1<<1)
#define GS_STANDBY_WAKEUP_IGNITION_INV      (1<<2)
#define GS_STANDBY_WAKEUP_CAN               (1<<3)
...
#define GS_CAN_FLAG_STD_ID                 ( 0 )        //!< Use 11 Bit ID
#define GS_CAN_FLAG_EXT_ID                 ( 1 << 0 )   //!< Use 29 Bit ID
#define GS_CAN_FLAG_RTR                    ( 1 << 1 )   //!< Remote Frame
#define GS_CAN_FLAG_ERR                    ( 1 << 2 )   //!< Error Frame
#define GS_CAN_FLAG_FD                     ( 1 << 3 )   //!< CanFD Frame
#define GS_CAN_FLAG_BRS                    ( 1 << 4 )   //!< Use Bitrate Switch

#define GS_CAN_STD_ID_MSK                   0x7FF
#define GS_CAN_EXT_ID_MSK                   0x1FFFFFFFUL
...
#define GS_SURF_TYPE_NONE                   -1
#define GS_SURF_TYPE_RGB555                 0
#define GS_SURF_TYPE_RGB24                  1
#define GS_SURF_TYPE_RGBA32                 2
#define GS_SURF_TYPE_RGB565                 3
#define GS_SURF_TYPE_YUV422_16              4
#define GS_SURF_TYPE_XYUV444_32             5
#define GS_SURF_TYPE_RGB555_ALPHA8          6
#define GS_SURF_TYPE_YUV420_YUV             7
#define GS_SURF_TYPE_GREY8                 10
...
//MsgContainerCutout
#ifdef MsgContainerCutout
#undef MsgContainerCutout
#endif
#define MsgContainerCutout _gs_api_MsgContainerCutoutTrampoline
...
//InfoContainerMove
#ifdef InfoContainerMove
#undef InfoContainerMove
#endif
#define InfoContainerMove _gs_api_InfoContainerMoveTrampoline

//InfoContainerCutout
#ifdef InfoContainerCutout
#undef InfoContainerCutout
#endif
#define InfoContainerCutout _gs_api_InfoContainerCutoutTrampoline
...
//GetVisibleInfoContainersPos
#ifdef GetVisibleInfoContainersPos
#undef GetVisibleInfoContainersPos
#endif
#define GetVisibleInfoContainersPos _gs_api_GetVisibleInfoContainersPosTrampoline
...
//GetCurrentMsgContainerShownPos
#ifdef GetCurrentMsgContainerShownPos
#undef GetCurrentMsgContainerShownPos
#endif
#define GetCurrentMsgContainerShownPos _gs_api_GetCurrentMsgContainerShownPosTrampoline
...
//CANRemoveRxFilter
#ifdef CANRemoveRxFilter
#undef CANRemoveRxFilter
#endif
#define CANRemoveRxFilter _gs_api_CANRemoveRxFilterTrampoline
...
#undef CANReadFrame
#endif
#define CANReadFrame _gs_api_CANReadFrameTrampoline

//CANSendFrame
#ifdef CANSendFrame
#undef CANSendFrame
#endif
#define CANSendFrame _gs_api_CANSendFrameTrampoline

//CANSendFrameWait
#ifdef CANSendFrameWait
#undef CANSendFrameWait
#endif
#define CANSendFrameWait _gs_api_CANSendFrameWaitTrampoline
...
//SetStatusLedColor
#ifdef SetStatusLedColor
#undef SetStatusLedColor
#endif
#define SetStatusLedColor _gs_api_SetStatusLedColorTrampoline
...
//UserSurfaceRequest
#ifdef UserSurfaceRequest
#undef UserSurfaceRequest
#endif
#define UserSurfaceRequest _gs_api_UserSurfaceRequestTrampoline

//UserSurfaceDelete
#ifdef UserSurfaceDelete
#undef UserSurfaceDelete
#endif
#define UserSurfaceDelete _gs_api_UserSurfaceDeleteTrampoline

//UserSurfaceGetAddr
#ifdef UserSurfaceGetAddr
#undef UserSurfaceGetAddr
#endif
#define UserSurfaceGetAddr _gs_api_UserSurfaceGetAddrTrampoline
...
/**
 * Number and x/y position of a container. This structure will be filled by
 * GetVisibleInfoContainersPos and GetCurrentMsgContainerShownPos
 */
typedef struct tagGsContPosInfo
{
    uint32_t mNum;      //!< Container number as configured in GSe-VISU
    int32_t mPosX;      //!< Current x position (may be INT32_MAX -> as in GSe-VISU given)
    int32_t mPosY;      //!< Current y position (may be INT32_MAX -> as in GSe-VISU given)
}tGsContPosInfo;

/**
* deprecated:
* Structure for sending and receiving CAN-Messages with CanReadFiFo and CanSendMsg
*
*/
...
enum eAnalogInType
{
    ...
    ANALOG_IN_TYPE_IGNITION,                //!< Analog input reads the ignition voltage
};
...
{
    uint64_t mTimestamp;         //!< Timestamp in nanoseconds
    uint32_t mId;                //!< Id of the message
    uint8_t  mChannel;           //!< The channel number
    uint8_t  mLen;               //!< Date length code (0-8)
    uint16_t mFlags;             //!< Use the flags GS_CAN_FLAG_... for different types of CAN-Messages.
    union
    {
        uint64_t u64[8];
        int64_t  s64[8];
...
typedef int32_t (*tCanFrameCallback)(tCanFrame * msg, void * user);
...
extern void _gs_api_MsgContainerCutoutTrampoline(int32_t contNum, int32_t cutFromTop, int32_t cutFromLeft, int32_t cutFromBottom, int32_t cutFromRight);
...
extern void _gs_api_InfoContainerMoveTrampoline(int32_t contNum, int32_t posX, int32_t posY);
extern void _gs_api_InfoContainerCutoutTrampoline(int32_t contNum, int32_t cutFromTop, int32_t cutFromLeft, int32_t cutFromBottom, int32_t cutFromRight);
extern uint32_t _gs_api_GetVisibleInfoContainersTrampoline(uint32_t * buf, uint32_t len);
extern uint32_t _gs_api_GetVisibleInfoContainersPosTrampoline(tGsContPosInfo * buf, uint32_t len);
...
extern int32_t _gs_api_GetCurrentMsgContainerShownPosTrampoline(tGsContPosInfo * info);
...
extern int _gs_api_CANAddRxFilterCallBackTrampoline(uint32_t channel, uint32_t id, uint32_t mask, uint8_t ext, tCanFrameCallback cb, void * user);
...
extern int32_t _gs_api_CANRemoveCallBackTrampoline(tCanFrameCallback cb);
extern int _gs_api_CANAddRxFilterFiFoTrampoline(uint32_t channel, uint32_t id, uint32_t mask, uint8_t ext, int32_t fifoId);
extern int _gs_api_CANRemoveRxFilterTrampoline(uint32_t channel, uint32_t id, uint32_t mask, uint8_t ext);
...
extern uint32_t _gs_api_CANReadFrameTrampoline(int32_t fifoId, tCanFrame * frames, uint32_t num, int32_t to);
extern int _gs_api_CANSendFrameTrampoline(tCanFrame * frame);
extern int _gs_api_CANSendFrameWaitTrampoline(tCanFrame * frame);
...
extern int32_t _gs_api_SetStatusLedColorTrampoline(uint32_t n, uint16_t r, uint16_t g, uint16_t b);
...
extern void _gs_api_UserSurfaceRequestTrampoline(uint32_t hdl, uint32_t channel, uint32_t picFmt,  uint32_t w, uint32_t h );
extern void _gs_api_UserSurfaceDeleteTrampoline(uint32_t channel);
extern void * _gs_api_UserSurfaceGetAddrTrampoline(const tCEvtSurfaceCreated * UsrSurf, uint32_t plane);
...
/**
 * Sets the cutout area for a message container. The container will be cut from
 * top/left/bottom/right by the number of given pixels. The position of the
 * container is not changed
 *
 * @param contNum           Number of the container
 * @param cutFromTop        Number of pixels cut from top side of container
 * @param cutFromLeft       Number of pixels cut from left side of container
 * @param cutFromBottom     Number of pixels cut from bottom side of container
 * @param cutFromRight      Number of pixels cut from rigth side of container
 */
void MsgContainerCutout(int32_t contNum, int32_t cutFromTop, int32_t cutFromLeft, int32_t cutFromBottom, int32_t cutFromRight);
...
/**
 * Moves an currently visible info container to a new position.
 * The z-order of all open containers will be kept when this function is used.
 * If the given container is not visible this function is a noop.
 *
 * @param contNum Number of the container to be moved.
 * @param posX    X position of the container
 * @param posY    Y position of the container
 */
void InfoContainerMove(int32_t contNum, int32_t posX, int32_t posY);

/**
 * Sets the cutout area for an info container. The container will be cut from
 * top/left/bottom/right by the number of given pixels. The position of the
 * container is not changed
 *
 * @param contNum           Number of the container
 ...
 * @param cutFromRight      Number of pixels cut from rigth side of container
 */
void InfoContainerCutout(int32_t contNum, int32_t cutFromTop, int32_t cutFromLeft, int32_t cutFromBottom, int32_t cutFromRight);
...
/**
 * Returns a list of currently visible info containers and their x/y positions
 *
 * @note Not available on gsOpcUaServer
 *
 * @param buf Buffer which will be filled with the visible info container numbers
 *            and their positions (May be NULL to get only the number of visible containers)
 * @param len Size of the buffer in number of container info structures
 *
 * @return The actual number of visible info containers
 */
uint32_t GetVisibleInfoContainersPos(tGsContPosInfo * buf, uint32_t len);
 ...
 * Returns the number of the currently visible message container
 *
 * @param info Will contain the informations about the current msg container.
 *             This parameter can be NULL.
 *
 * @return The number of the currently visible message container
 *         or -1 if no message container is visible
 */
int32_t GetCurrentMsgContainerShownPos(tGsContPosInfo * info);
 ...
 *
 * int32_t canReceive(tCanFrame * msg)
 * {
 *     doSomething();
 *     return 0;
 * }
 ...
 * @param user    A pointer which is given to the callback function when it is called
 *
 * @return 1 if the callback function was successfully registered
 */
int CANAddRxFilterCallBack(uint32_t channel, uint32_t id, uint32_t mask, uint8_t ext, tCanFrameCallback cb, void * user);
 ...
 */
int32_t CANRemoveCallBack(tCanFrameCallback cb);
...
/**
 * Removes a CAN id/mask combination from reception.
 *
 * @note Not available on gsOpcUaServer
 *
 * @param channel The can channel for which the id should be disabled.
 * @param id      The id for the filter
 * @param mask    The mask for the filter
 * @param ext     != 0 if id/mask is a 29 bit id
 *
 * @return 0 if the filter was successfully removed
 */
int CANRemoveRxFilter(uint32_t channel, uint32_t id, uint32_t mask, uint8_t ext);
...
/**
 * Reads up to num can frames from the given can fifo and stores them in the given array of can
 * message structures.
 *
 * @note Not available on gsOpcUaServer
 *
 * @param fifoId Id of the fifo as returned by CANCreateFiFo or 0 if you want to use the
 *               global fifo.
 * @param frames A pointer to num can frames. The function will store the received can frames
 *               here.
 * @param num    Number of can frames available at frames.
 *
 * @param to     If >0 : The Function will block until num messages have been read, or the Timeout is reached.
 *               If  0 : The Function will read all available messages from the CAN and return immediately.
 *               If <0 : The Function will block until num messages have been received in the fifo.
 * @return The number of actual frames copied to msgs. This can be 0 or up to num.
 */
uint32_t CANReadFrame(int32_t fifoId, tCanFrame * frames, uint32_t num, int32_t to);
 ...
 * @note Not available on gsOpcUaServer
 *
 * @param frame The can frame to be send
 *
 * @return 1 if the message was successfully put to the send FiFo
 */
int CANSendFrame(tCanFrame * frame);

/**
 * Sends a can frame and waits until it is passed to the hardware.
 * This function is only needed in some special cases. CANSendFrame()
 * should be perfect in most cases.
 *
 * @note Not available on gsOpcUaServer
 *
 * @param frame The can frame to be send
 *
 * @return 1 if the frame was successfully passed to the hardware.
 */
int CANSendFrameWait(tCanFrame * frame);
...
/**
 * Sets the color of a status led. If multiple status LEDs are available n selects the
 * LED to be set
 *
 * @param n Number of the status led
 * @param r The RED value of the color (0..65535)
 * @param g The GREEN value of the color (0..65535)
 * @param b The BLUE value of the color (0..65535)
 *
 * @return 0 if the status led color was set successfully or < 0 if there was an
 *         error.
 */
int32_t SetStatusLedColor(uint32_t n, uint16_t r, uint16_t g, uint16_t b);
...
/**
 * Requests a new user surface in the grafic memory of the device. The surface will be shown
 * in a video channel instead of an analog video stream. You will get the surface
 * by the eventsystem (type == tCEvtSurfaceCreated). Give this event to "GetUserSurfaceAddr"
 * as a parameter. It will return you the adress of the new created user surface. You can add
 * serveral surfaces to one video channel. By changing the input of the video channel, you
 * can change the shown surface.
 *
 * @note Not available on gsOpcUaServer
 *
 * @param hdl        handle to identify the requested surface.
 * @param channel    video channel which will display this surface
 * @param picFmt     pciture format of the surface.
 *                   Only GS_PIC_FMT_* - should be used here.
 * @param w          width of the surface in pixel
 * @param h          number of lines of the surface
 */
void UserSurfaceRequest(uint32_t hdl, uint32_t channel, uint32_t picFmt,  uint32_t w, uint32_t h );

/**
 * Deltes all user surfaces of one channel. After deleting the surfaces,
 * the analoug video channels are used as input, again.
 *
* @note Not available on gsOpcUaServer
 *
 * @param channel    the video channel whose  UserSufraces you want to delte.
 **/
void UserSurfaceDelete(uint32_t channel);

/**
 * Returns the address of a User-Surface created by ReqNewSurface
 *
 *  @param UsrSurf Event gernerated, after requesting a new surface
 *  @param plane   If using the format GS_SURF_TYPE_YUV420_YUV,
 *                 the picture is saved in 3 layers. Use
 *                 0 for Y-Layer
 *                 1 for U-Layer
 *                 2 for V-Layer.
 *  @return address of the UserSurface. If using GS_SURF_TYPE_YUV420_YUV as format,
 *          it will return the Y,U or V layer depending on the parameter plane.
 */
void * UserSurfaceGetAddr(const tCEvtSurfaceCreated * UsrSurf, uint32_t plane);


#endif //USERCAPI_PROTOTYPES

/**
 * Always false as we are in the file defining this. Only our comment parser
 * should see this.
 */
#if !defined(USERCAPI_H_INCLUDED)

/**
 * Following protottypes are for functions available in standard C. To have
 * a nice description on GSe-VISU for them we redeclare them here with some
 * descriptive comments. Compiler will never see these due to the condition
 * above.
 */
...
#endif //!defined(USERCAPI_H_INCLUDED)
